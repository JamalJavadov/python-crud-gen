...
- Generates production-grade CRUD for selected entities:
  ...
  - DTOs (Create/Update/Patch/Response) with validations (+ version for concurrency)
  - MapStruct Mapper (builder disabled; relations ignored on write; ids mapped on read)
  - Service with relation resolving (IDs), list/search filters, soft delete, version check -> 409
  - Controller (CRUD + pageable list/search, POST Location header)
- Common layer:
  - ApiEnvelope (avoids swagger ApiResponse clash)
  - PageResponse
  - ApiError + ErrorCode (incl CONFLICT)
...
BOT_MARKER = "GENERATED BY java-project-crud.py"
...
def dto_names(ent: Entity) -> Tuple[str, str, str, str]:
    return (
        f"{ent.name}CreateRequestDto",
        f"{ent.name}UpdateRequestDto",
        f"{ent.name}PatchRequestDto",
...
def build_dto_fields(ent: Entity, emap: Dict[str, Entity]) -> Tuple[List[str], List[str], List[str], List[str], Set[str]]:
    ...
    resp_lines.append("    private Long version;")

    imports.add("jakarta.validation.constraints.NotNull")
    update_lines.append("    @NotNull")
    update_lines.append("    private Long version;")
    patch_lines.append("    @NotNull")
    patch_lines.append("    private Long version;")
    ...
    for f in ent.fields:
        ...
        if f.is_rel_single and f.rel_target:
            ...
            anns_p, imps_p = validation_for_field(fake, for_patch=True)
            imports |= imps_c | imps_p

            for a in anns_c: create_lines.append(f"    {a}")
            create_lines.append(f"    private {idt} {name};")

            for a in anns_c: update_lines.append(f"    {a}")
            update_lines.append(f"    private {idt} {name};")

            for a in anns_p: patch_lines.append(f"    {a}")
            patch_lines.append(f"    private {idt} {name};")
        ...
        if f.is_rel_multi and f.rel_target:
            ...
            create_lines.append(f"    private List<{idt}> {name};")
            update_lines.append(f"    private List<{idt}> {name};")
            patch_lines.append(f"    private List<{idt}> {name};")
        ...
        anns_p, imps_p = validation_for_field(f, for_patch=True)
        imports |= imps_u | imps_p

        for a in anns_u: create_lines.append(f"    {a}")
        create_lines.append(f"    private {t} {f.name};")

        for a in anns_u: update_lines.append(f"    {a}")
        update_lines.append(f"    private {t} {f.name};")

        for a in anns_p: patch_lines.append(f"    {a}")
        patch_lines.append(f"    private {t} {f.name};")

        resp_lines.append(f"    private {t} {f.name};")

    return create_lines, update_lines, patch_lines, resp_lines, imports

def gen_dtos(proj: Project, ent: Entity, emap: Dict[str, Entity]) -> List[Tuple[Path, str]]:
    pkg = dto_pkg(proj, ent)
    create_name, update_name, patch_name, resp_name = dto_names(ent)
    create_lines, update_lines, patch_lines, resp_lines, imps = build_dto_fields(ent, emap)
...
"""
    ...
    return [
        (dto_dir / f"{create_name}.java", dto_code(create_name, create_lines)),
        (dto_dir / f"{update_name}.java", dto_code(update_name, update_lines)),
        (dto_dir / f"{patch_name}.java", dto_code(patch_name, patch_lines)),
...
"""
    ...
    files: List[Tuple[Path, str]] = [
        (to_pkg_dir(proj.src_main_java, common) / "ApiEnvelope.java", api_envelope),
        (to_pkg_dir(proj.src_main_java, common) / "PageResponse.java", page_response),
...
import io.swagger.v3.oas.annotations.enums.SecuritySchemeIn;"""
        ...
        openapi_cfg = f"""// {BOT_MARKER}
package {proj.base_package};
...
def gen_mapper(proj: Project, ent: Entity, emap: Dict[str, Entity]) -> Tuple[Path, str]:
    ...
    patch_maps = [f'    @Mapping(target = "{x}", ignore = true)' for x in ignores]
    ...
    for f in ent.fields:
        if f.is_rel_single and f.rel_target and f.rel_target in emap:
            ...
            patch_maps.append(f'    @Mapping(target = "{f.name}", ignore = true)')
            target = emap[f.rel_target]
...
{chr(10).join(create_maps)}
    {ent.name} toEntity({create_name} dto);

{chr(10).join(update_maps)}
    void updateFromUpdateDto(@MappingTarget {ent.name} entity, {update_name} dto);

    @BeanMapping(nullValuePropertyMappingStrategy = NullValuePropertyMappingStrategy.IGNORE)
{chr(10).join(patch_maps)}
    void updateFromPatchDto(@MappingTarget {ent.name} entity, {patch_name} dto);
...
def gen_service(proj: Project, ent: Entity, emap: Dict[str, Entity]) -> Tuple[Path, str]:
    ...
    imports: Set[str] = {
        ...
        f"{exc_pkg}.BadRequestException",
        f"{exc_pkg}.ConflictException",
...
public class {ent.name}Service {{
    ...
    public {resp_name} create({create_name} dto) {{
...
{chr(10).join(resolve_create)}
    ...
    public {resp_name} update({ent.id_type} id, {update_name} dto) {{
...
{chr(10).join(resolve_update)}
    ...
    public {resp_name} patch({ent.id_type} id, {patch_name} dto) {{
        {ent.name} entity = repository.findById(id)
                .orElseThrow(() -> new {ent.name}NotFound(id));
...
{chr(10).join(resolve_patch)}
        entity = repository.save(entity);
        return mapper.toDto(entity);
    }}

    @Transactional(readOnly = true)
    public {resp_name} get({ent.id_type} id) {{
        {ent.name} entity = repository.findById(id)
                .orElseThrow(() -> new {ent.name}NotFound(id));
        return mapper.toDto(entity);
    }}
    ...
    @Transactional(readOnly = true)
    public Page<{resp_name}> list(Map<String, String> params, Pageable pageable) {{
        var spec = {ent.name}Specifications.fromParams(params);
        return repository.findAll(spec, pageable).map(mapper::toDto);
    }}

    @Transactional(readOnly = true)
    public Page<{resp_name}> search(Map<String, String> params, Pageable pageable) {{
...
def gen_controller(proj: Project, ent: Entity) -> Tuple[Path, str]:
    ...
    id_getter = f"get{camel(ent.id_field)}"

    imports: Set[str] = {
        f"{common}.ApiEnvelope",
        f"{common}.PageResponse",
        f"{dtoP}.{create_name}",
        f"{dtoP}.{update_name}",
        f"{dtoP}.{patch_name}",
        f"{dtoP}.{resp_name}",
    ...
    if proj.with_openapi:
        ...
        swagger = f'@Tag(name = "{ent.name}", description = "{ent.name} CRUD API")\n'

    def op_line(summary: str, desc: str = "") -> str:
        if not proj.with_openapi:
            return ""
        if desc:
            return f'    @Operation(summary = "{summary}", description = "{desc}")\n'
...
{op_line(f"Create {ent.name}")}    @PostMapping
    public ResponseEntity<ApiEnvelope<{resp_name}>> create(@Valid @RequestBody {create_name} dto) {{
        {resp_name} created = service.create(dto);
        URI location = URI.create("{route}" + "/" + created.{id_getter}());
        return ResponseEntity.created(location).body(ApiEnvelope.ok("created", created));
    }}

{op_line(f"Replace {ent.name} (PUT)", "Full replace")}    @PutMapping("/{{id}}")
    public ResponseEntity<ApiEnvelope<{resp_name}>> update(@PathVariable {ent.id_type} id, @Valid @RequestBody {update_name} dto) {{
        return ResponseEntity.ok(ApiEnvelope.ok("updated", service.update(id, dto)));
    }}

{op_line(f"Partial update {ent.name} (PATCH)", "Partial update (null ignored)")}    @PatchMapping("/{{id}}")
    public ResponseEntity<ApiEnvelope<{resp_name}>> patch(@PathVariable {ent.id_type} id, @Valid @RequestBody {patch_name} dto) {{
        return ResponseEntity.ok(ApiEnvelope.ok("updated", service.patch(id, dto)));
    }}

{op_line(f"Get {ent.name} by id")}    @GetMapping("/{{id}}")
    public ResponseEntity<ApiEnvelope<{resp_name}>> get(@PathVariable {ent.id_type} id) {{
        return ResponseEntity.ok(ApiEnvelope.ok("ok", service.get(id)));
    }}

{op_line(f"List {ent.name}", "Filters via query params: q, field, minField/maxField, fromField/toField, relationId, relationIds")}    @GetMapping
    public ResponseEntity<ApiEnvelope<PageResponse<{resp_name}>>> list(@RequestParam Map<String, String> params, Pageable pageable) {{
        Page<{resp_name}> page = service.list(params, pageable);
        return ResponseEntity.ok(ApiEnvelope.ok("ok", PageResponse.from(page)));
    }}

{op_line(f"Search {ent.name}", "Alias for list with same filters")}    @GetMapping("/search")
    public ResponseEntity<ApiEnvelope<PageResponse<{resp_name}>>> search(@RequestParam Map<String, String> params, Pageable pageable) {{
        Page<{resp_name}> page = service.search(params, pageable);
        return ResponseEntity.ok(ApiEnvelope.ok("ok", PageResponse.from(page)));
    }}

{op_line(f"Delete {ent.name} (soft delete)")}    @DeleteMapping("/{{id}}")
